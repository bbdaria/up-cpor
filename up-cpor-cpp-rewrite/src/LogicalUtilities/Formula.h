...
public:
    ...
    virtual bool is_true(const PredicateSet& known, bool contains_negations) const = 0;
    virtual bool is_false(const PredicateSet& known, bool contains_negations) const = 0;
    virtual bool is_true_delete_relaxation(const PredicateSet& known) const = 0;

    bool is_true(const PredicateSet& known) const { 
        return this->is_true(known, false); 
    }
    bool is_false(const PredicateSet& known) const { 
    ...
    virtual void get_all_predicates(PredicateSet& predicates) const = 0;
    virtual void get_all_effect_predicates(PredicateSet& conditional_predicates, 
                                           PredicateSet& non_conditional_predicates) const = 0;
    ...
    virtual bool contained_in(const PredicateSet& predicates, bool contains_negations) const = 0;
    ...
    virtual std::shared_ptr<Formula> regress(std::shared_ptr<PlanningAction> a, const PredicateSet& observed) = 0;
    virtual std::shared_ptr<Formula> regress(std::shared_ptr<PlanningAction> a) = 0;
    virtual std::shared_ptr<Formula> reduce(const PredicateSet& known) = 0;

    virtual bool contains_non_deterministic_effect() const = 0;
    virtual int get_max_non_deterministic_options() const = 0;
    virtual void get_all_optional_predicates(PredicateSet& predicates) const = 0;
    ...
    virtual std::shared_ptr<Formula> remove_impossible_options(const PredicateSet& observed) = 0;
    virtual std::shared_ptr<Formula> apply_known(const PredicateSet& known) = 0;
    ...
    virtual std::shared_ptr<Formula> reduce_conditions(const PredicateSet& known) = 0;
