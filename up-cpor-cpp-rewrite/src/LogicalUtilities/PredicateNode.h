...
class PredicateNode : public Formula {
private:
...
public:
    ...
    std::shared_ptr<Predicate> get_predicate() const { return predicate_; }
    using Formula::is_true;
    using Formula::is_false;
    virtual Type get_type() const override { return Type::Predicate; }
    ...
    virtual bool contains_condition() const override { return false; }
    ...
    virtual std::shared_ptr<Formula> regress(std::shared_ptr<PlanningAction> a, const PredicateSet& observed) override;
    ...
    virtual void get_all_optional_predicates(PredicateSet& predicates) const override;
    ...
    virtual std::shared_ptr<Formula> remove_impossible_options(const PredicateSet& observed) override;
    virtual std::shared_ptr<Formula> apply_known(const PredicateSet& known) override;
    ...
    virtual std::shared_ptr<Formula> reduce_conditions(const PredicateSet& known) override;
